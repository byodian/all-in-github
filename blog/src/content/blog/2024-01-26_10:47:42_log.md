...
modDatetime: 2024-01-26T02:47:42Z
---
mybatis-plus 将 Java 对象保存为 json 类型的数据。`typeHandler` 类型处理器，用于 JavaType 与 JdbcType 之间的转换

通过 TableField注解快速注入到 mybatis 容器中。使用注解的方式需要开启映射注解 `@TableName(autoResultMap = true)`
  实际测试发现 mybatis-plus BaseMapper 接口中的方法会使用 `typeHandler ` 处理 JavaType 与 JdbcType 的转换。
